function colorirFonteTitulo(str) {
...
function colorirFonteEscolher(str) {
    let array = ["colorirVermelho", "colorirVerde", "colorirAmarelo", "colorirAzul"]
    let cont = 0;
    let constante = document.createElement("div");
    constante.classList.add("palavra")
    const divTitulo = document.querySelector(".titulo-escolher")
    for (let i = 0; i < str.length; i++) {
        const p = document.createElement("p");
        if (str[i] == " ") {
            divTitulo.appendChild(constante)
...
colorirFonteTitulo("Jogo Da Velha")

colorirFonteEscolher("Escolher Personagem")